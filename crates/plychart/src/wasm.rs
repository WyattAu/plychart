...
pub fn update_radar_multi(
    ...
    let theme = parse_theme(theme_json);
    let series: Vec<RadarSeries> =
        serde_json::from_str(data_json).map_err(|e| JsValue::from_str(&e.to_string()))?;
    let labels: Vec<String> =
        serde_json::from_str(labels_json).map_err(|e| JsValue::from_str(&e.to_string()))?;
...
pub fn destroy_chart(canvas_id: &str) -> Result<(), JsValue> {
    crate::canvas::destroy_chart(canvas_id).map_err(|e| JsValue::from_str(&e.to_string()))
}
