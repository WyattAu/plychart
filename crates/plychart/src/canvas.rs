...
pub fn update_order_book(canvas_id: &str, data_json: &str) -> Result<(), crate::ChartError> {
    ...
    let v: serde_json::Value = serde_json::from_str(data_json)
        .map_err(|e| crate::ChartError::DataParseError(e.to_string()))?;
    let bids = v.get("bids").and_then(|b| b.as_array()).map(|a| {
        a.iter().filter_map(|p| {
            let arr = p.as_array()?;
            Some((arr.get(0)?.as_f64()?, arr.get(1)?.as_f64()?))
        }).collect::<Vec<_>>()
    }).unwrap_or_default();
    let asks = v.get("asks").and_then(|a| a.as_array()).map(|a| {
        a.iter().filter_map(|p| {
            let arr = p.as_array()?;
            Some((arr.get(0)?.as_f64()?, arr.get(1)?.as_f64()?))
        }).collect::<Vec<_>>()
...
#[cfg(target_arch = "wasm32")]
pub fn update_gauge(canvas_id: &str, value: f64, max: f64, color: &str) -> Result<(), crate::ChartError> {
    use wasm_bindgen::JsCast;
    use web_sys::CanvasRenderingContext2d;
    let window = web_sys::window().ok_or(crate::ChartError::CanvasNotFound("No window".into()))?;
    let document = window.document().ok_or(crate::ChartError::CanvasNotFound("No document".into()))?;
...
#[cfg(not(target_arch = "wasm32"))]
pub fn update_gauge(_canvas_id: &str, _value: f64, _max: f64, _color: &str) -> Result<(), crate::ChartError> {
    Ok(())
}

/// Update chart with backtest equity + drawdown data.
#[cfg(target_arch = "wasm32")]
pub fn update_backtest(canvas_id: &str, equity_json: &str, drawdown_json: &str) -> Result<(), crate::ChartError> {
    use wasm_bindgen::JsCast;
    use web_sys::CanvasRenderingContext2d;
    let window = web_sys::window().ok_or(crate::ChartError::CanvasNotFound("No window".into()))?;
    let document = window.document().ok_or(crate::ChartError::CanvasNotFound("No document".into()))?;
...
#[cfg(not(target_arch = "wasm32"))]
pub fn update_backtest(_canvas_id: &str, _equity_json: &str, _drawdown_json: &str) -> Result<(), crate::ChartError> {
    Ok(())
}

/// Update chart with radar data.
#[cfg(target_arch = "wasm32")]
pub fn update_radar(canvas_id: &str, values_json: &str, labels_json: &str, color: &str) -> Result<(), crate::ChartError> {
    use wasm_bindgen::JsCast;
    use web_sys::CanvasRenderingContext2d;
    let window = web_sys::window().ok_or(crate::ChartError::CanvasNotFound("No window".into()))?;
    let document = window.document().ok_or(crate::ChartError::CanvasNotFound("No document".into()))?;
...
#[cfg(not(target_arch = "wasm32"))]
pub fn update_radar(_canvas_id: &str, _values_json: &str, _labels_json: &str, _color: &str) -> Result<(), crate::ChartError> {
    Ok(())
}
...
#[cfg(target_arch = "wasm32")]
pub fn update_correlation(canvas_id: &str, matrix_json: &str, labels_json: &str) -> Result<(), crate::ChartError> {
    use wasm_bindgen::JsCast;
    use web_sys::CanvasRenderingContext2d;
    let window = web_sys::window().ok_or(crate::ChartError::CanvasNotFound("No window".into()))?;
    let document = window.document().ok_or(crate::ChartError::CanvasNotFound("No document".into()))?;
...
#[cfg(not(target_arch = "wasm32"))]
pub fn update_correlation(_canvas_id: &str, _matrix_json: &str, _labels_json: &str) -> Result<(), crate::ChartError> {
    Ok(())
}
        ...
        if let Some(document) = window.document() {
            if let Some(element) = document.get_element_by_id(canvas_id) {
                if let Ok(canvas) = element.dyn_into::<web_sys::HtmlCanvasElement>() {
                    ...
                    if let Some(ctx) = canvas.get_context("2d")
